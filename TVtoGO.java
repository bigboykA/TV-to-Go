...
public class TVtoGO implements ActionListener, ListSelectionListener {
	...
	/**
	 * Constructs a TVtoGO object which creates a graphical user 
	 * interface to select, search, and  download movies and episodes of TV series
	 * @throws FileNotFoundException when the file to populate the collection of
	 * movies and episodes is not found
	 */
	...
	private static ArrayList<VideoItem> readFile(Scanner input) {
		...
		while (input.hasNextLine()) {
			...
			if (type.equals("M")) {
				Movie movie = new Movie(data[0], data[1], data[2], Integer.parseInt(data[3]),
						Integer.parseInt(data[4]), Double.parseDouble(data[5]), Integer.parseInt(data[6]));
